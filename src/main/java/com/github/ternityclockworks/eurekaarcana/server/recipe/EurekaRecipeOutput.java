...
public class EurekaRecipeOutput {

	public static final EurekaRecipeOutput EMPTY = new EurekaRecipeOutput(ItemStack.EMPTY, 1);
	...
	public EurekaRecipeOutput(ItemStack stack, float chance) {
		this.stack = stack;
		this.chance = chance;
	}

	public EurekaRecipeOutput(Pair<ResourceLocation, Integer> item, float chance) {
	...
	public static EurekaRecipeOutput deserialize(JsonElement je) {
		if (!je.isJsonObject())
			throw new JsonSyntaxException("EurekaRecipeOutput must be a json object");
		...
		return new EurekaRecipeOutput(itemstack, chance);
	}
	...
	public static EurekaRecipeOutput read(FriendlyByteBuf buf) {
		return new EurekaRecipeOutput(buf.readItem(), buf.readFloat());
	}
