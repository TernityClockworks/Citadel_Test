...
import com.github.ternityclockworks.eurekaarcana.util.VecHelper;
import com.github.ternityclockworks.eurekaarcana.server.misc.EurekaSoundRegistry;
import com.github.ternityclockworks.eurekaarcana.client.render.item.CustomItemUseEffect;
//import com.github.ternityclockworks.eurekaarcana.client.render.item.EurekaItemRenderProperties;
import com.github.ternityclockworks.eurekaarcana.mixin.accessor.LivingEntityAccessor;
import com.github.ternityclockworks.eurekaarcana.server.recipe.ManualCombinationRecipe;
...
public class BrokenMechanismItem extends Item implements CustomItemUseEffect {

	public BrokenMechanismItem(Properties properties) {
	...
	public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
		...
		if (ManualCombinationRecipe.canCombine(worldIn, itemInOtherHand)) {
			ItemStack item = itemInOtherHand.copy();
			ItemStack toCombine = item.split(1);
			playerIn.startUsingItem(handIn);
			itemstack.getOrCreateTag()
				.put("Combining", toCombine.serializeNBT());
		...
		for (ItemEntity itemEntity : worldIn.getEntitiesOfClass(ItemEntity.class, bb)) {
			...
			if (!ManualCombinationRecipe.canCombine(worldIn, stack))
				continue;
			pickUp = itemEntity;
			break;
		}
	...
	public ItemStack finishUsingItem(ItemStack stack, Level worldIn, LivingEntity entityLiving) {
		...
		if (tag.contains("Combining")) {
			ItemStack toCombine = ItemStack.of(tag.getCompound("Combining"));
			ItemStack polished =
				ManualCombinationRecipe.combine(worldIn, entityLiving.position(), toCombine, stack);
	...
	public static void spawnParticles(Vec3 location, ItemStack combiningStack, Level world) {
		for (int i = 0; i < 20; i++) {
			Vec3 motion = VecHelper.offsetRandomly(Vec3.ZERO, world.random, 1 / 8f);
			world.addParticle(new ItemParticleOption(ParticleTypes.ITEM, combiningStack), location.x, location.y,
	...
	public void releaseUsing(ItemStack stack, Level worldIn, LivingEntity entityLiving, int timeLeft) {
		...
		if (tag.contains("Combining")) {
			ItemStack toCombine = ItemStack.of(tag.getCompound("Combining"));
			player.getInventory()
				.placeItemBackInInventory(toCombine);
			tag.remove("Combining");
		}
	}
	...
	*/
	...
	public boolean triggerUseEffects(ItemStack stack, LivingEntity entity, int count, RandomSource random) {
		CompoundTag tag = stack.getOrCreateTag();
		if (tag.contains("Combining")) {
			ItemStack combining = ItemStack.of(tag.getCompound("Combining"));
			((LivingEntityAccessor) entity).create$callSpawnItemParticles(combining, 1);
		}
	...
	//	return EurekaSoundRegistry.MECHANISM_DISASSEMBLING.get();
	//}
